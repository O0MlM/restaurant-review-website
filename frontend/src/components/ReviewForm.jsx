...
function ReviewForm({ restaurantId, onReviewAdded }) {
  ...
  const validateForm = () => {
    ...
    else if (comment.length < 10) newErrors.comment = 'ความคิดเห็นต้องมีอย่างน้อย 10 ตัวอักษร';
    else if (comment.length > 500) newErrors.comment = 'ความคิดเห็นต้องไม่เกิน 500 ตัวอักษร';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(error.message || 'เกิดข้อผิดพลาดในการส่งรีวิว');
    } finally {
      setSubmitting(false);
    }
  };
